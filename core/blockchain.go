...
// ExecResult is a data structure to store execution result after every
// Reference block. We prune this once once a state-commit transaction
// is included in the reference-chain.
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
			...
			for refNum <= currRef {
				...
				if dc == nil || !dc.Status {
					...
					case <-bc.foreignDataCh:
						log.Info("Received foreignDataCh")
						continue
					}
				}
				refNum++
		...
		receipts, privateReceipts, logs, usedGas, err := bc.processor.Process(block, bc.foreignData, bc.pendingCrossTxs, startRef, currRef, state, privateState, bc.vmConfig)
		if err != nil {
