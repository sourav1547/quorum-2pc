...
type (
	...
	CanTransferFunc func(*types.TxControl, *types.RWLock, *types.RWLock, uint64, StateDB, common.Address, *big.Int) bool
...
type EVM struct {
	...
	gLocked   *types.RWLock // globally locked keys
	cUnlocked *types.RWLock // locally unlocked keys
...
func NewEVM(ctx Context, tcb *types.TxControl, gLocked, cUnlocked *types.RWLock, statedb, privateState StateDB, chainConfig *params.ChainConfig, vmConfig Config) *EVM {
	evm := &EVM{
		Context:      ctx,
		tcb:          tcb,
		gLocked:      gLocked,   // global locked keys
		cUnlocked:    cUnlocked, // keys unlocked so far due to transaction execution
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if !evm.Context.CanTransfer(evm.tcb, evm.gLocked, evm.cUnlocked, evm.Context.Shard, evm.StateDB, caller.Address(), value) {
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// Fail if we're trying to transfer more than the available balance
	if !evm.CanTransfer(evm.tcb, evm.gLocked, evm.cUnlocked, evm.Context.Shard, evm.StateDB, caller.Address(), value) {
		return nil, gas, ErrInsufficientBalance
	}
...
func getBalance(env *EVM, addr common.Address) (*big.Int, error) {
	...
	/**
	if gLock, gaok := env.gLocked.Locks[addr]; gaok {
		if _, uaok := env.cUnlocked.Locks[addr]; !uaok {
			log.Info("@ds, addresslocked found during balance check!", "addr", addr)
			return nil, ErrAddressLocked
		}
		uLock := env.cUnlcoked.Locks[addr]
