...
func New(eth Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(block *types.Block) bool, commitments map[uint64]types.Commitments, pendingCrossTxs map[uint64]types.CrossShardTxs, myLatestCommit *types.Commitment, pendingResults map[uint64]*core.ExecResult, pendingResultsMu, commitLock, crossTxsLock sync.RWMutex) *Miner {
	miner := &Miner{
		eth:      eth,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(config, engine, eth, mux, recommit, gasFloor, gasCeil, isLocalBlock, commitments, pendingCrossTxs, myLatestCommit, pendingResults, pendingResultsMu, commitLock, crossTxsLock),
		canStart: 1,
	}
	go miner.update()
	go miner.rUpdate()
