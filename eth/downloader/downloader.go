...
var (
	...
	errNoSyncActiveR           = errors.New("no reference sync active")
...
func New(ref bool, mode SyncMode, stateDb ethdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, myshard uint64) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "ref", d.myref, "eth", p.version, "head", hash, "td", td, "mode", d.mode)
		...
		case packet := <-d.headerCh:
			...
			for i := 0; i < len(headers); i++ {
				if number := headers[i].Number.Int64(); number != from+int64(i)*16 {
					p.log.Warn("@is Head headers broke chain ordering", "index", i, "requested", from+int64(i)*16, "received", number)
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if skeleton {
				...
				if err != nil {
					p.log.Warn("@is Skeleton chain invalid", "err", err)
		...
		case packet := <-deliveryCh:
			...
			if peer := d.cousinPeers[shard].Peer(packet.PeerId()); peer != nil {
				...
				if err == errInvalidChain {
					log.Warn("@is Error in delivering packet")
					return err
				}
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					if n, err := d.lightchain.InsertHeaderChain(chunk, frequency); err != nil {
						...
						log.Warn("@is Invalid header encountered", "number", chunk[n].Number, "hash", chunk[n].Hash(), "err", err)
						return errInvalidChain
					}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		log.Warn("@is Downloaded item processing failed", "number", results[index].Header.Number, "hash", results[index].Header.Hash(), "err", err)
		return errInvalidChain
	}
	return nil
}
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if skeleton {
				filled, proced, err := d.fillHeaderSkeleton(from, headers)
				if err != nil {
					log.Warn("@is skeleton chain invalid", "peer", p, "err", err)
					return errInvalidChain
				}
				headers = filled[proced:]
				from += uint64(proced)
