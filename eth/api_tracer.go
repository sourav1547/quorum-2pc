...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			_, _, _, _, err := api.eth.blockchain.Processor().Process(block, nil, nil, uint64(0), uint64(0), statedb, privateStateDb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, nil, statedb, privateStateDb, api.config, vm.Config{})
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, privateStateDb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, nil, statedb, privateStateDb, api.config, vm.Config{Debug: true, Tracer: tracer})
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(context, nil, statedb, privateStateDb, api.config, vm.Config{})
